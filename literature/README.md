...
本地已下载13篇公开全文。编号不代表重要性排名。第三问核心阅读为04、07、08、10、11，预测继承修订新增15号在线教材；第四问使用12、13、14。01、03、09的鲁棒方法不进入当前模型。
...
### 14 Flexible Operation of Batteries in Power System Scheduling With Renewable Energy

- 作者：Nan Li、Canan Uçkun、Emil M. Constantinescu、John R. Birge、Kory W. Hedman、Audun Botterud。
- 年份：2016，IEEE Transactions on Sustainable Energy 7(2), 685–696。
- DOI：[10.1109/TSTE.2015.2497470](https://doi.org/10.1109/TSTE.2015.2497470)；[实验室公开全文](https://www.mcs.anl.gov/papers/P6089-0416.pdf)。
- 状态：已在线阅读全文中的第II-A节（分桶非预见性）和第II-D节（储能灵活运行）。命令行下载遇到站点验证，未保存本地PDF；本地已下载数量仍为13，不把在线阅读写成下载完成。
- 用途：Q4-3实质修订中“用少量信息组表示下一更新后的条件行动”及“未来灵活性影响当前储电”的思路来源。
- 适配限制：不照搬原文风电分桶、机组组合、备用市场、软库存区间及其罚金。本题组号只能依赖下一节点确实可观测的预报修订和已实现价格偏差；原始承诺、共同桥接库存与只前瞻一次更新的具体模型另行定义。Q4-2的依赖收缩公式不是该文献结论。

以下资料未下载，逐项记录原因和阅读范围。此前NREL域名存在DNS解析失败，相关链接保留供浏览器直接访问。

### 15 Forecasting: Principles and Practice（第3版）第13.4节：Forecast combinations

- 作者：Rob J Hyndman、George Athanasopoulos。
- 类型：作者公开在线教材第3版；本次核对在线章节，不为动态网页另指定出版年份。
- 来源：[第13.4节预测组合](https://otexts.com/fpp3/combinations.html)。
- 状态：已在线阅读全文本节；未下载新的PDF，本地公开全文数量仍为13。
- 用途：第三问预测继承修订中，保留历史预测、融合校准官方预报、设置简单平均基准，并注意预测误差相关性。
- 适配限制：本题采用有限权重候选及因果残差日块，不能照搬教材示例的收益；未复现其软件实现，也不将融合宣称为必然改善或学术首创。
...
6. 第四问先读12的回测规范、13的优化信息需求，再读14的II-A、II-D理解本次下一节点前瞻；Q4-2收缩的具体公式以模型文档为准。
