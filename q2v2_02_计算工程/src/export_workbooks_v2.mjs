...
const checks=[];
// 只在交付表中去除1e-9以下的求解浮点噪声，避免显示“-0.00”；原始轨迹保留。
const clean=value=>typeof value==='number' && Math.abs(value)<1e-9 ? 0 : value;
const datesToExcel=rows=>rows.map(row=>[row[0]===null?null:new Date(`${row[0]}T00:00:00Z`),...row.slice(1).map(clean)]);
const formatOnly=process.argv.includes('--format-only');
const q2Only=process.argv.includes('--q2-only');
...
for (const name of formatOnly||q2Only?['result2']:['result1','result2']) {
  ...
  if(formatOnly) {
    ...
    finishExtendedFormatting(workbook);
    const notes=[payload.notes.time_mapping,payload.notes.method,
      payload.notes.q2_total_columns,'模型：model-q1-q2-revised.pdf；数据：附件1、2；各电量为kWh，费用为元。'];
    for(let j=0;j<notes.length;j++) {
      const row=337+j;
      const range=purchase.getRange(`A${row}:H${row}`);
      range.merge();
      purchase.getRange(`A${row}`).values=[[notes[j]]];
      range.format.font={name:'宋体',size:10};
      range.format.wrapText=true;
      range.format.rowHeight=32;
    }
    previews=[['计划购电量','A1:E7'],['计划购电量','EN330:EQ335'],['充放电量','A1:F13'],
              ['充放电量','A2000:F2005'],['紧急购电量','A1:C12'],['计划购电量','A337:H340']];
...
await fs.writeFile(path.join(qa,formatOnly?'format_checks.json':q2Only?'artifact_checks_q2_revision.json':'artifact_checks.json'),JSON.stringify(checks,null,2));
