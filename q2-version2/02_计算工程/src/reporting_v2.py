...
def build_payload(run_path,dataset,config,main_folder,method_note):
    run=Path(run_path)
    q1=json.loads((run/"q1.json").read_text(encoding="utf-8"))["milp"]
    with np.load(run/main_folder/"trajectories.npz") as saved:
    ...
    payload={"q1":{"purchases":[[interval_label(t),a["Q"][t]] for t in range(144)],
             ...
             "notes":{"time_mapping":"按源标签为区间终点解释；输出区间为00:00-00:10至23:50-24:00，点序和数值不循环平移。",
